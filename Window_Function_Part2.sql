-- Understanding Window Function Part2
...
from (
    ...
    CONCAT(CAST(ROUND(cume_dist() OVER (ORDER BY price DESC) * 100, 2) AS VARCHAR), '%') as cume_dist_percentage
    from product) x
...
from (
    ...
    CONCAT(CAST(ROUND(percent_rank() OVER (ORDER BY price) * 100, 2) AS VARCHAR), '%') as per
